...
from spacebridge_mcp.server import (
    ...
    spacebridge_client as server_spacebridge_client, # Import with alias to avoid fixture conflict
    openai_client as server_openai_client,          # Import with alias
    get_git_info,
    perform_version_check,
    get_config_value,
    main_sync # Import main_sync for testing config loading
)
from spacebridge_mcp.spacebridge_client import SpaceBridgeClient # Import class for type hints
...
async def test_get_issue_tool_handler_not_found():
    ...
    mock_client_instance.get_issue.side_effect = Exception("Simulated client error (e.g., 404)")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"org": "startup_org", "proj": "startup_proj"}, {}, {"org": "startup_org", "proj": "startup_proj"}),
        # Case 2: Startup context exists, tool context exists -> Use startup context (priority)
        ({"org": "startup_org", "proj": "startup_proj"}, {"org": "tool_org", "proj": "tool_proj"}, {"org": "startup_org", "proj": "startup_proj"}),
        # Case 3: No startup context, tool context exists -> Use tool context
        ({}, {"org": "tool_org", "proj": "tool_proj"}, {"org": "tool_org", "proj": "tool_proj"}),
        # Case 4: No startup context, no tool context -> Use None
        ({}, {}, {"org": None, "proj": None}),
        # Case 5: Partial startup context (org only) -> Use startup org, fallback proj
        ({"org": "startup_org", "proj": None}, {"org": "tool_org", "proj": "tool_proj"}, {"org": "startup_org", "proj": "tool_proj"}),
        # Case 6: Partial startup context (proj only) -> Use startup proj, fallback org
        ({"org": None, "proj": "startup_proj"}, {"org": "tool_org", "proj": "tool_proj"}, {"org": "tool_org", "proj": "startup_proj"}),
        # Case 7: Partial tool context -> Use startup context where available, fallback to tool context
        ({"org": "startup_org", "proj": None}, {"org": None, "proj": "tool_proj"}, {"org": "startup_org", "proj": "tool_proj"}),
    ]
)
async def test_search_issues_handler_context_priority(startup_context, tool_context, expected_call_context):
...
@pytest.mark.asyncio
@patch('spacebridge_mcp.server.openai_client', new_callable=AsyncMock) # Mock global openai client
async def test_create_issue_handler_new(mock_openai_client_instance):
    ...
    tool_context = {"org": "tool_org", "proj": "tool_proj"} # Tool context should be ignored
    expected_call_context = startup_context # Startup context has priority
    ...
    mock_sb_client_instance.create_issue.return_value = {
        "id": new_issue_id, "title": title, "description": description, "status": "New", "url": new_issue_url
    }
...
async def test_update_issue_handler_success(startup_context, tool_context, expected_call_context):
...
@patch('spacebridge_mcp.server.perform_version_check', return_value=True) # Assume version check passes
@patch('spacebridge_mcp.server.app.run') # Mock app run
def test_main_sync_config_precedence(
    mock_app_run, mock_version_check, mock_openai_init, mock_sb_client_init,
    mock_get_git_info, mock_os_getenv, mock_os_path_exists, mock_load_dotenv, mock_parse_args
):
    """Test the configuration precedence within main_sync."""

    # --- Test Case 1: Args take highest precedence ---
    mock_parse_args.return_value = argparse.Namespace(
        spacebridge_api_url="arg_url", spacebridge_api_key="arg_key", openai_api_key="arg_openai",
        org_name="arg_org", project_name="arg_proj", project_dir=None
    )
    mock_os_getenv.side_effect = lambda key, default=None: {
        "SPACEBRIDGE_API_URL": "env_url", "SPACEBRIDGE_API_KEY": "env_key", "OPENAI_API_KEY": "env_openai",
        "SPACEBRIDGE_ORG_NAME": "env_org", "SPACEBRIDGE_PROJECT_NAME": "env_proj"
    }.get(key, default)
    ...
    mock_sb_client_init.assert_called_with(
        api_url="arg_url", api_key="arg_key", org_name="arg_org", project_name="arg_proj"
    )
    ...
    mock_parse_args.return_value = argparse.Namespace(
        spacebridge_api_url=None, spacebridge_api_key=None, openai_api_key=None,
        org_name=None, project_name=None, project_dir=None # No args
    )
    ...
    mock_sb_client_init.assert_called_with(
        api_url="env_url", api_key="env_key", org_name="env_org", project_name="env_proj"
    )
    ...
    mock_parse_args.return_value = argparse.Namespace(
        spacebridge_api_url="arg_url", spacebridge_api_key="arg_key", openai_api_key="arg_openai", # Use args for API keys
        org_name=None, project_name=None, project_dir=None # No org/proj args
    )
    ...
    mock_sb_client_init.assert_called_with(
        api_url="arg_url", api_key="arg_key", org_name="git_org", project_name="git_proj" # API keys from args, context from git
    )
    mock_openai_init.assert_called_with(api_key="arg_openai")
    ...
    mock_parse_args.return_value = argparse.Namespace(
        spacebridge_api_url="arg_url", spacebridge_api_key="arg_key", openai_api_key="arg_openai",
        org_name=None, project_name=None, project_dir="/custom/path" # Use project-dir
    )
    ...
    mock_sb_client_init.assert_called_with(
        api_url="arg_url", api_key="arg_key", org_name="git_org_custom", project_name="git_proj_custom"
    )
    mock_openai_init.assert_called_with(api_key="arg_openai")
    # Assert get_git_info called with the custom path
    mock_get_git_info.assert_called_once_with(os.path.join("/custom/path", ".git/config"))
...
def test_perform_version_check_compatible(mock_meta_version):
    ...
    mock_client_instance.get_version.return_value = {
        "server_version": "1.0.0", "min_client_version": "0.1.0", "max_client_version": "0.2.0"
    }
