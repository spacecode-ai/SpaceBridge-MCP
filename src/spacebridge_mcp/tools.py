...
from mcp.types import JSONRPCRequest # Keep for potential type hints if needed elsewhere, though likely unused now
...
class IssueSummary(BaseModel):
    ...
    description: Optional[str] = Field(None, description="The description of the issue (might be truncated in search results).") # Added description field
    url: Optional[str] = Field(None, description="Direct URL to the issue in the tracker.")
    score: Optional[float] = Field(None, description="Relevance score (for similarity search).")

class SearchIssuesOutput(BaseModel):
    results: List[IssueSummary] = Field(..., description="A list of issues matching the search query.")
...
class CreateIssueOutput(BaseModel):
    ...
    status: Literal["created", "existing_duplicate_found"] = Field(..., description="Indicates if a new issue was created or a duplicate was found.")
    message: str = Field(..., description="A message describing the outcome.")
    url: Optional[str] = Field(None, description="Direct URL to the created/found issue.")
...
class UpdateIssueOutput(BaseModel):
    ...
    status: Literal["updated", "failed"] = Field(..., description="Indicates if the update was successful.")
    message: str = Field(..., description="A message describing the outcome.")
